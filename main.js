...
function createItemMesh(type) {
    ...
    switch (type) {
        ...
        case 'talisman': {
            ...
            // 부적 실루엣 트레이스 v4 (팔/다리 균형)
            const s = new THREE.Shape();
            s.moveTo(-0.09, 0.20);   // 왼쪽 뿔 끝
            s.lineTo(-0.04, 0.13);   // 왼쪽-중앙 V노치
            s.lineTo(0.00, 0.23);    // 중앙 뿔 끝
            s.lineTo(0.04, 0.13);    // 중앙-오른쪽 V노치
            s.lineTo(0.09, 0.20);    // 오른쪽 뿔 끝
            s.lineTo(0.14, 0.17);    // 오른쪽 어깨
            s.lineTo(0.21, 0.09);    // 오른쪽 팔 바깥 위 (팔 축소)
            s.lineTo(0.21, 0.01);    // 오른쪽 팔 바깥 아래
            s.lineTo(0.12, -0.05);   // 오른쪽 허리 상단
            s.lineTo(0.10, -0.10);   // 오른쪽 허리
            s.lineTo(0.22, -0.15);   // 오른쪽 다리 바깥 위 (다리 확장)
            s.lineTo(0.22, -0.26);   // 오른쪽 다리 끝 바깥 (더 깊게)
            s.lineTo(0.12, -0.26);   // 오른쪽 다리 끝 안
            s.lineTo(0.06, -0.13);   // 가랑이 오른쪽
            s.lineTo(-0.06, -0.13);  // 가랑이 왼쪽
            s.lineTo(-0.12, -0.26);  // 왼쪽 다리 끝 안
            s.lineTo(-0.22, -0.26);  // 왼쪽 다리 끝 바깥
            s.lineTo(-0.22, -0.15);  // 왼쪽 다리 바깥 위
            s.lineTo(-0.10, -0.10);  // 왼쪽 허리
            s.lineTo(-0.12, -0.05);  // 왼쪽 허리 상단
            s.lineTo(-0.21, 0.01);   // 왼쪽 팔 바깥 아래
            s.lineTo(-0.21, 0.09);   // 왼쪽 팔 바깥 위
            s.lineTo(-0.14, 0.17);   // 왼쪽 어깨
            s.closePath();

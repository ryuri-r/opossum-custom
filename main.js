...
function createItemMesh(type) {
    ...
    switch (type) {
        ...
        case 'talisman': {
            ...
            // 부적 실루엣 트레이스 v3 (3뿔 + 사다리꼴 팔 + 두 다리)
            const s = new THREE.Shape();
            s.moveTo(-0.09, 0.20);   // 왼쪽 뿔 끝
            s.lineTo(-0.04, 0.13);   // 왼쪽-중앙 V노치
            s.lineTo(0.00, 0.23);    // 중앙 뿔 끝 (제일 높음)
            s.lineTo(0.04, 0.13);    // 중앙-오른쪽 V노치
            s.lineTo(0.09, 0.20);    // 오른쪽 뿔 끝
            s.lineTo(0.15, 0.17);    // 오른쪽 어깨 안쪽
            s.lineTo(0.23, 0.09);    // 오른쪽 팔 바깥 위 (대각선 모서리)
            s.lineTo(0.23, 0.01);    // 오른쪽 팔 바깥 아래
            s.lineTo(0.14, -0.04);   // 오른쪽 몸통
            s.lineTo(0.12, -0.09);   // 오른쪽 허리
            s.lineTo(0.20, -0.17);   // 오른쪽 다리 바깥 위
            s.lineTo(0.20, -0.24);   // 오른쪽 다리 끝 바깥
            s.lineTo(0.11, -0.24);   // 오른쪽 다리 끝 안
            s.lineTo(0.06, -0.12);   // 가랑이 오른쪽
            s.lineTo(-0.06, -0.12);  // 가랑이 왼쪽
            s.lineTo(-0.11, -0.24);  // 왼쪽 다리 끝 안
            s.lineTo(-0.20, -0.24);  // 왼쪽 다리 끝 바깥
            s.lineTo(-0.20, -0.17);  // 왼쪽 다리 바깥 위
            s.lineTo(-0.12, -0.09);  // 왼쪽 허리
            s.lineTo(-0.14, -0.04);  // 왼쪽 몸통
            s.lineTo(-0.23, 0.01);   // 왼쪽 팔 바깥 아래
            s.lineTo(-0.23, 0.09);   // 왼쪽 팔 바깥 위
            s.lineTo(-0.15, 0.17);   // 왼쪽 어깨 안쪽
            s.closePath();
            ...
            hl.position.set(0.012, 0.055, 0.036);
            g.add(hl);
